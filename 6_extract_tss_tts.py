...
def extract_regions(genes, tss_path, tts_path):
	...
	Tools.create_folder("/".join(str(tss_path).split("/")[:-1]))
	...
	Tools.create_folder("/".join(str(tts_path).split("/")[:-1]))
